...
const demoTrace = [
  ...
  },
  {
    ...
    ip: "151.101.1.69",
    rtt: 68.4,
    role: "CDN Edge / Destination Network Edge",
    city: "New York",
    country: "United States",
    country_code: "US",
    asn: "AS54113",
...
function stopStream() {
  ...
  if (demoTimer) {
    ...
    demoTimer = null;
    drawStatus("Stopped", "Demo trace cancelled");
  }
}

function startDemoTrace(target) {
  ...
  setStatus(`Demo: ${target}`);
  drawStatus("Demo", "Static demo trace");

  let idx = 0;
  demoTimer = setInterval(() => {
    if (idx >= demoTrace.length) {
      setStatus("Complete");
      drawStatus("Complete", `Demo trace ${run.id} finished`);
      stopStream();
      return;
    }
    const hop = { ...demoTrace[idx], hop: idx + 1 };
    drawHop(run, hop);
    idx += 1;
  }, 450);
}
...
document.addEventListener("DOMContentLoaded", async () => {
  initMap();
  if (demoMode) {
    drawStatus("Demo", "Static demo. Run the backend for live traces.");
  }
